...
test('anonymous email linking does not prematurely send a password', async () => {
  ...
  assert.deepEqual(payload, { email: 'demo@example.invalid' });
  assert.equal(options.emailRedirectTo, 'http://localhost:4173/');
});
test('anonymous or unverified users cannot set a permanent login password', async () => {
  ...
  await assert.rejects(
    setVerifiedPassword(
      client,
      { email: 'demo@example.invalid', is_anonymous: true },
      'test-only-password',
    ),
  );
  await assert.rejects(
    setVerifiedPassword(
      client,
      { email: 'demo@example.invalid', is_anonymous: false },
      'test-only-password',
    ),
  );
});
