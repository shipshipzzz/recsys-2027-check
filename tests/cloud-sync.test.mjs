...
test('cloud writes are scoped to the authenticated user and return server acknowledgements',async()=>{
  ...
  const client={from(table){assert.equal(table,'user_card_states');return {
    upsert(value,opts){payload=value;options=opts;return {
      select(value){columns=value;return {abortSignal(value){signal=value;return Promise.resolve({data:ack,error:null});}};}
    };}
  };}};
...
test('cloud reads are explicitly filtered to one user',async()=>{
  ...
  const client={from(table){assert.equal(table,'user_card_states');return {
    select(value){selected=value;return {
      eq(column,value){filter=[column,value];return {abortSignal(value){signal=value;return Promise.resolve({data:rows,error:null});}};}
    };}
  };}};
