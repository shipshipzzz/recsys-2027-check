...
for (const kind of ['rec', 'soe']) {
  test(kind + ': every current and historical field survives database roundtrip', () => {
    ...
    for (const item of items)
      checkSubset(
        item,
        actual.ALL_ITEMS.find((x) => x.id === item.id),
        kind + '/' + item.name,
      );
...
test('normalized catalog has complete stable references and never imports private states', () => {
  const count = Object.values(catalogs).reduce(
    (n, d) => n + d.DATA.length + (d.EXTRA?.length || 0),
    0,
  );
  ...
  assert.equal(tables.user_card_states, undefined);
  assert.equal(TABLE_NAMES.length, 10);
});
test('fallback reconstructs identical array references used by UI helpers', () => {
  const data = finalizeCatalog({ DATA: [{ id: 'one' }], EXTRA: [{ id: 'two' }] });
  assert.equal(data.ALL_ITEMS[0], data.DATA[0]);
  assert.equal(data.ALL_ITEMS[1], data.EXTRA[0]);
});
