...
export async function verifyDeployment({
  ...
  const expected = new Map(
    files.map((file) => [file, sha256(fs.readFileSync(path.join(directory, file)))]),
  );
  ...
  async function read(file) {
    ...
    assert.equal(
      sha256(bytes),
      expected.get(file),
      `Published content differs from this build: ${file}`,
    );
    return bytes;
  }
...
async function main() {
  ...
  assert.equal(
    ...
    digest(normalizeCatalogs(loadCatalogs())),
    'Rebuild after editing local catalog data',
  );
  const receipt = await verifyDeployment({
    directory,
    revision,
    onRetry: (attempt, error) =>
      console.log(`Pages verification retry ${attempt}: ${safeError(error)}`),
  });
  fs.mkdirSync(path.join(ROOT, 'test-results'), { recursive: true });
  fs.writeFileSync(
    path.join(ROOT, 'test-results/pages-verification.json'),
