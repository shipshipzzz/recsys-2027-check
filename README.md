...
## 最近重点核查（北京时间 2026-09-16）

本次复核并更新 **6 张现有卡片**：推荐页的哔哩哔哩、vivo，以及央国企页的招银网络科技、工行软件开发中心、工行深圳分行、杭州银行。保留 111 张卡片及全部稳定 ID 和原始快照；其余卡片不冒充本次重新验岗。新增 9 条来源，专业及地区评估随事实同步复核。

招银科技第二批 9/20 18:00 由用人单位简章支持；工行两个入口补入正文明确的 10/8 报名节点。杭州银行两版公告出现 10/22 与 10/25、境外毕业窗和流程差异，暂不设硬倒计时；B站 FAQ 未给统一截止，取消卡片的第三方 10/3 倒计时（原始时间线保留待核历史）；vivo 9/15 历史待核节点已过，改为当前需复核，不能据网页正文未读成判停招。

逐项证据、读取限制和验收见 [本次招聘核查记录](docs/recruitment-review-2026-09-16.md)。招聘核查阶段仅在本地完成；后续发布状态以对应 GitHub Actions 运行和线上 `release.json` 验收为准。
...
当前资料包括 **111 张卡片**（推荐页 57、国企页 54）、110 个公司实体、83 张历史卡片快照、65 个来源、49 个固定时间线节点和 5 组更新日志。动态「今天」标记在浏览器按北京时间生成，不写入固定时间线。
