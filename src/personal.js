import { supabase, readableError, config } from './backend.js';
...
export function createPersonalManager(
  kind,
  catalog,
  { renderer = createCardRenderer(), onRefreshCatalog = () => {} } = {},
) {
  const storage = safeStorage();
  const namespace = new URL(config.url).hostname.replace(/\.supabase\.co$/, '');
  const makeStore = (scope = 'guest') => new StateStore(storage, scope, () => {}, { namespace });
  ...
  let disposed = false,
    ...
    authEventVersion = 0,
    formBusy = false;
  let catalogStatus = { refreshing: false, failed: false },
    summarySignature = '',
    pullFlight = null;
  const retry = createRetryScheduler(async () => {
    await pull();
  });
  function queueRetry(error) {
    if (!disposed && user && navigator.onLine && isRetryableError(error)) retry.schedule();
  }
  let epoch = 0,
  ...
  filterLabel.innerHTML =
    '<span>我的状态</span><select aria-label="筛选我的卡片状态" data-personal-filter><option value="all">全部状态</option><option value="active">只看未处理</option><option value="applied">只看已投递</option><option value="uninterested">只看不感兴趣</option></select>';
  document.querySelector('.filters').append(filterLabel);
  const processed = document.createElement('section');
  processed.id = 'processed-wrap';
  processed.hidden = true;
  processed.innerHTML =
    '<div class="processed-heading"><h2>已处理的卡片 <span data-processed-count></span></h2><p>这里只改变你的个人视图，不代表招聘已结束。点击“恢复”即可回到前面的列表。</p></div><div id="processed-grid"></div>';
  ...
  toast.className = 'personal-toast';
  toast.hidden = true;
  ...
  function rerender() {
    ...
    rendering = true;
    const focused = document.activeElement?.closest('[data-card-id]');
    const focus = focused ? { id: focused.dataset.cardId, state: focused.dataset.cardState } : null;
    const open = [...document.querySelectorAll('.card details[open]')].map((el) => ({
      id: el.closest('.card')?.dataset.entryId,
      index: [...el.closest('.card').querySelectorAll('details')].indexOf(el),
    }));
    try {
      onChange();
      for (const x of open) {
        const card = document.querySelector(`[data-entry-id="${x.id}"]`);
        const detail = card?.querySelectorAll('details')[x.index];
    ...
    if (focus) {
      const card = document.querySelector(`[data-entry-id="${focus.id}"]`);
      (
        card?.querySelector(`[data-card-state="${focus.state}"]`) ||
        card?.querySelector('[data-card-state]')
      )?.focus({ preventScroll: true });
    }
  }
  function update() {
    if (disposed) return;
    const counts = { active: 0, applied: 0, uninterested: 0 };
    for (const item of items) counts[store.status(item.id)]++;
    const nextSummary = Object.values(counts).join(':');
    if (nextSummary !== summarySignature) {
      summarySignature = nextSummary;
      panel.querySelector('.personal-summary').innerHTML = Object.entries(counts)
        .map(([s, n]) => `<span><b>${n}</b>${labels[s]}</span>`)
        .join('');
    }
    panel.querySelector('[data-catalog-mode]').textContent =
      ({ cloud: '云端招聘资料', cached: '离线缓存资料', fallback: '内置只读备份' }[
        catalog.connection
      ] || '本地资料') +
      ' · ' +
      items.length +
        ...
        : user.is_anonymous
          ...
          : realtimeStatus === 'SUBSCRIBED'
            ...
            : realtimeStatus === 'CHANNEL_ERROR' || realtimeStatus === 'TIMED_OUT'
              ? '云端已同步 · 实时连接重试中'
              : '云端已同步 · 跨设备同步';
    if (user && !hasRemoteSnapshot) text = '正在读取账号状态…';
    if (store.lastError) text = '同步失败 · 本机记录保留';
    if (store.inFlight) text = '正在同步…';
    if (store.storageError) text = '本地存储不可用或损坏，请勿关闭页面';
    ...
    panel.querySelector('[data-import]').hidden =
      !user || !Object.values(makeStore().entries).some((e) => e.status !== 'active');
    panel.querySelector('[data-account]').textContent =
      user?.email && !user?.is_anonymous ? '已绑定邮箱' : '邮箱登录 / 绑定';
    panel.querySelector('[data-identity-note]').textContent = !user
      ? '无需登录即可标记。启用云端同步后，当前浏览器的标记会保存到数据库。'
      : user.is_anonymous
        ? '当前为匿名设备账号：刷新不会丢失，但清除浏览器数据或退出后无法找回。绑定邮箱并设置密码后，才可在其他设备登录同一账号同步。'
        : `账号：${user.email} · 在其他设备登录同一邮箱账号后会自动拉取标记；在线设备会实时更新。`;
    panel.querySelectorAll('button').forEach((b) => (b.disabled = busy));
    panel.querySelector('[data-enable-sync]').disabled = busy || !authReady;
    panel.querySelector('[data-refresh-catalog]').disabled = busy || catalogStatus.refreshing;
  ...
  function notify(message, undo = false) {
    ...
    if (undo) {
      ...
      b.addEventListener(
        'click',
        () => {
          if (lastAction && lastAction.scope === store.scope) {
  ...
  async function startRealtime(target = store, version = epoch) {
    ...
    try {
      const subscription = await subscribeUserStates(supabase, target.scope, {
        ...
        onStatus: (status, error) => {
          ...
          if (status === 'SUBSCRIBED') scheduleRemotePull(target, version);
            ...
            console.warn('跨设备实时同步连接异常，将继续通过刷新/聚焦重试。', error);
        },
      });
      if (version !== epoch || store !== target) {
      ...
      if (version === epoch && store === target) {
        ...
        update();
        console.warn('跨设备实时同步初始化失败，将继续通过刷新/聚焦补拉。', error);
      }
    }
  }
  async function sync(target = store, version = epoch) {
    if (disposed || target.scope === 'guest' || version !== epoch || store !== target) return false;
    try {
      await target.flush((batch) => writeUserStateBatch(supabase, target.scope, batch), {
        shouldContinue: () => !disposed && version === epoch && store === target,
      });
      return target.pendingCount() === 0;
    } catch (error) {
      if (store === target && !disposed) {
  ...
  function pull(target = store, version = epoch) {
    if (disposed || target.scope === 'guest' || version !== epoch || store !== target)
      return Promise.resolve();
    if (pullFlight?.target === target) {
      pullFlight.again = true;
      return pullFlight.promise;
    }
    const flight = { target, again: false, promise: null };
    flight.promise = (async () => {
      if (target.pendingCount() && !(await sync(target, version))) return;
      if (disposed || version !== epoch || store !== target) return;
      const baseline = target.capture();
      const data = await readUserStates(supabase, target.scope);
      if (disposed || version !== epoch || store !== target) return;
      target.lastError = null;
      hasRemoteSnapshot = true;
      target.acceptRemote(data, { baseline });
      lastRefresh = Date.now();
      if (target.pendingCount()) await sync(target, version);
      if (!target.lastError && !target.pendingCount()) retry.reset();
    })()
      .catch((error) => {
        if (!disposed && version === epoch && store === target) {
      ...
      .finally(() => {
        ...
        if (flight.again && !disposed && version === epoch && store === target)
          scheduleRemotePull(target, version);
      });
    pullFlight = flight;
  ...
  async function adoptSession(session) {
    ...
    // Change ownership synchronously, before awaiting old channel cleanup.
    const version = ++epoch;
    retry.reset();
    lastAction = null;
    toast.hidden = true;
    ...
    ready = (async () => {
      ...
      if (disposed || version !== epoch || store !== target) return;
      if (next) {
        void startRealtime(target, version);
        await pull(target, version);
      }
    })();
    return ready;
  }
  ...
  panel.querySelector('[data-refresh-catalog]').addEventListener(
    'click',
    () => {
  ...
  panel.querySelector('[data-enable-sync]').addEventListener(
    'click',
    () =>
      withBusy(async () => {
        ...
        notify(
          store.pendingCount()
            ? '云端账号已启用，部分标记等待同步。'
            : '云端同步已启用。建议绑定邮箱，避免清除浏览器数据后失去账号。',
        );
      }),
    { signal: lifetime.signal },
  );
  panel.querySelector('[data-retry]').addEventListener(
    'click',
    () =>
      withBusy(async () => {
        retry.reset();
  ...
  panel.querySelector('[data-import]').addEventListener(
    'click',
    () =>
      withBusy(async () => {
  ...
  panel.querySelector('[data-signout]').addEventListener(
    ...
    () =>
      withBusy(async () => {
        ...
        if (
          user?.is_anonymous &&
          !confirm('当前为匿名账号。退出后无法重新登录此账号，建议先绑定邮箱。仍要退出？')
        )
          return;
  ...
  panel.querySelector('[data-account]').addEventListener(
    'click',
    () => {
      dialog.querySelector('.auth-feedback').textContent = '';
      dialog.querySelector('[name=email]').value = user?.email || '';
      dialog.showModal();
      dialog.querySelector('[name=email]').focus();
    },
    { signal: lifetime.signal },
  );
  dialog
    .querySelector('[data-close]')
    .addEventListener('click', () => dialog.close(), { signal: lifetime.signal });
  ...
  dialog.querySelector('form').addEventListener(
    ...
    async (event) => {
      ...
      const form = event.currentTarget,
        ...
        feedback = dialog.querySelector('.auth-feedback');
      ...
      try {
        if (button?.value === 'password') {
          await setVerifiedPassword(supabase, user, password);
          feedback.textContent = '登录密码已设置。现在可以在其他设备用此邮箱和密码登录。';
          ...
          await adoptSession(data.session);
          dialog.close();
          notify('已登录。云端标记已读取，本机访客标记不会自动混入账号。');
        } else {
          const redirect = new URL(location.pathname, location.origin).href;
          if (user?.is_anonymous) {
            await requestEmailLink(supabase, email, redirect);
            feedback.textContent =
              '确认邮件已请求发送。请在此浏览器打开邮件链接，再回到账号窗口点击“设置 / 修改登录密码”。匿名标记会保留在同一账号。';
          } else if (user) {
            throw new Error('当前账号已经绑定邮箱。需要切换账号时，请退出后登录。');
          } else {
            ...
            if (data.session) {
              ...
              feedback.textContent =
                '请检查邮箱中的确认邮件。确认后登录，再使用“同步本机标记”导入已有记录。';
          }
        }
      } catch (error) {
  ...
  filterLabel.querySelector('select').addEventListener(
    'change',
    (event) => {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      const previous = store.status(id),
        next = button.dataset.cardState;
      if (next === previous) return;
      retry.reset();
      lastAction = { id, status: previous, scope: store.scope };
      store.set(id, next);
      notify(
        `${byId.get(id).name}：${next === 'active' ? '已恢复到未处理列表' : labels[next] + '，已置灰并移到末尾'}`,
        true,
      );
      void sync();
    },
  ...
  window.addEventListener(
    'focus',
    () => {
      if (user && Date.now() - lastRefresh > 15000) void pull();
    },
    { signal: lifetime.signal },
  );
  document.addEventListener(
    'visibilitychange',
    () => {
      if (!document.hidden && user && Date.now() - lastRefresh > 15000) void pull();
    },
    { signal: lifetime.signal },
  );
  store.notify = update;
  const manager = {
    ...
    reportStartupError() {
      notify('账号初始化暂时不可用，招聘资料和本机标记仍可使用。请检查网络后刷新。');
    },
    dispose() {
      disposed = true;
      epoch++;
    ...
    async start(callback) {
      ...
      const subscription = supabase.auth.onAuthStateChange((_event, session) => {
        authEventVersion++;
        clearTimeout(authTimer);
        // Macrotask, not a microtask: leave the auth library's lock before API calls.
        authTimer = setTimeout(() => {
          authTimer = null;
          if (disposed) return;
          authReady = true;
          void adoptSession(session).catch(() => manager.reportStartupError());
        }, 0);
      });
      authSubscription = subscription.data.subscription;
      const observed = authEventVersion;
      const { data, error } = await withTimeout(supabase.auth.getSession(), 10000, '账号读取超时');
      if (disposed) return;
      if (error) {
        notify(readableError(error));
        return;
      }
      if (observed === authEventVersion) {
        authReady = true;
        await adoptSession(data?.session);
      }
    },
    snapshot: () => ({
      ...
      states: items.map((x) => ({ id: x.id, name: x.name, status: store.status(x.id) })),
    }),
  };
  return manager;
}
